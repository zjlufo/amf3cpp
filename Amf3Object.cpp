...
template <class T>
void amf3reflist<T>::AddObj(char * key, T obj)
{
	int len = strlen(key);
	char * pkey = new char[len+1];
	memset(pkey, 0, len+1);
	memcpy(pkey, key, len);
	propnames.push_back(pkey);
	properties.push_back(obj);
}

template <class T>
T & amf3reflist<T>::GetObj(char * key)
{
	//vector<char *>::const_iterator iter;
	//iter = propnames.begin();
	for (unsigned int i = 0; i < propnames.size(); /*++iter,*/ ++i)
	{
		if (!strcmp(propnames[i], key))
...
template <class T>
int amf3reflist<T>::Exists(char * key)
{
	//vector<char *>::const_iterator iter;
	//iter = propnames.begin();
	for (unsigned int i = 0; i < propnames.size(); /*++iter,*/ ++i)
...
amf3classdef::amf3classdef(char * name, vector<char *> & properties, bool dynamic, bool externalizable)
{
	//		amf3classdefcreate++;
	if (name != 0)
	{
...
void amf3object::InternalCopy(const amf3object &val) {
	...
	case String:
		{
			_value.text = new char[strlen(val._value.text)+1];
			memset(_value.text, 0, strlen(val._value.text)+1);
			memcpy(_value.text, val._value.text, strlen(val._value.text));
			break;
		}
	case Array:
		{
...
amf3object& amf3object::operator=(const char *pVal) {
	Reset();
	type = String;
	_value.text = new char[strlen(pVal)+1];
	memset(_value.text, 0, strlen(pVal)+1);
	memcpy(_value.text, pVal, strlen(pVal));
	return *this;
}
...
amf3object& amf3object::operator=(const string &val) {
	Reset();
	type = String;
	_value.text = new char[val.length()+1];
	memset(_value.text, 0, val.length()+1);
	memcpy(_value.text, val.c_str(), val.length());
	return *this;
}
...
amf3object::operator char*()
{
	...
	case Integer:
		{
			char * ret;
			asprintf(&ret, "%d", this->operator int32_t());
			return ret;
	...
	case Number:
		{
			char * ret;
			asprintf(&ret, "%.3f", this->operator double());
			return ret;
...
bool amf3object::operator==(amf3object variant) {
	...
	if (type == String)
	{
		if (this->_value.text && variant._value.text)
			return !strcmp(this->_value.text, variant._value.text);
		else if (!this->_value.text && !variant._value.text)
			return true;
		else
			return false;
	}
...
amf3object  amf3parser::ReadNextObject(void)
{
	...
	case String:
		...
		{
			len = strlen(pstr);
			obj._value.text = new char[len+1];
			memset(obj._value.text, 0, len+1);
			memcpy(obj._value.text, pstr, len);
		}
		else
		{
			obj._value.text = new char[13];
			strcpy(obj._value.text, "<NULLSTRING>");
		}
		return obj;
	case Date:
		obj._value.date = ReadDate();
		return obj;
	case Array:
...
char * amf3parser::ReadString(void)
{
	int num = ReadInteger();
	...
	if (len > 0)
	{
		if ((num & 1) == 0)
		{
			return (char*)(strlist.GetObj(num >> 1));
		}
...
//	amfreflist * tempnames = 0;
	...
	for (int i = 0; i < classdef.properties.size(); i++)
	{
		name = (char *)(classdef.properties.at(i));
		if (!strcmp(name, "userName"))
			int i = 0;//__debugbreak();
		//Log("Associative: %s being added", name);
		tempobject = ReadNextObject();
		encapslist.AddObj(tempobject);

		//		if (!strcmp(name, "cmd"))
		//		{ Log("Command: %s", tempobject->string); }
...
//			__asm {int 3};
	...
	if (classdef.dynamic)
	{
		...
		while (key != 0)
		{
			...
			//internalstrlist.AddObj(key);

			//			if (!strcmp(key, "cmd"))
			//			{ Log("Command: %s", temp->string); }
...
void amf3writer::Write(amf3object & obj)
{
	...
	if (obj.type == String)
	{
		Write(obj._value.text);
		return;
	}
	if (obj.type == Date)
	{
		Write(Date);
		TypelessWrite(obj._value.date);
		return;
	}
	if (obj.type == Array)
	{
...
void amf3writer::Write(char * str)
{
	Write(String);
	TypelessWrite(str);
}
...
void amf3writer::TypelessWrite(char * str)
{
	...
	//Need UTF8 code here...
	TypelessWrite( (int)(strlen(str) << 1 | 1) );
	strcpy(stream+position, str);
	position += strlen(str);
	...
	stringTable.insert( Int_Pair (stringTable.size(), temp) );
	strlist.AddObj(temp);
}
void amf3writer::WriteDictionary(amf3reflist<amf3object> * reflist)
{
	...
	{
		TypelessWrite((char*)reflist->propnames.at(i));
...
void amf3writer::TypelessWrite(amf3objectmap * _object, amf3object & obj)
{
	...
	if (!found)
	{
		...
		TypelessWrite((char*)_object->classdef.name);

		for (int i = 0; i < _object->classdef.properties.size(); ++i)
		{
			TypelessWrite((char*)(_object->classdef.properties.at(i)));
		}
	}
...
amf3object & amf3objectmap::Get(char * key)
{
	return properties.GetObj(key);
}
void amf3objectmap::Add(char * key, amf3object & obj)
{
	properties.AddObj(key, obj);
}
int amf3objectmap::Exists(char * key)
{
...
void amf3array::Add(char * key, amf3object & obj)
{
	this->type = 2;
...
amf3object & amf3array::Get(char * key)
{
...
inline bool amf3object::IsEqual (amf3object & obj, amf3object & obj2)
{
	...
	case 5:
		...
		break;
	case 6:
		if (!strcmp(obj._value.text, obj2._value.text))
			return true;
		break;
	case 7:
		return false;
	case 8:
		if (!strcmp(obj._value.date, obj2._value.date))
			return true;
		break;
	case 9:
